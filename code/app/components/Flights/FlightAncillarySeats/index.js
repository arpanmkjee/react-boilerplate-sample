import React from 'react';


function FlightAncillarySeats() {
    return (
        <div class="ancillary-main show">
            <div class="seatSection make_relative">
                <div class="ancillary-banner LatoBold" id="seatAncBanner" style={{ top: "164px" }}>
                    <img src="https://imgak.mmtcdn.com/flights/assets/media/dt/ancillaries/seats_green_strip_icon.png?v=1"
                        style={{ marginRight: "10px", height: "20px" }} /><span>
                        Enjoy a comfortable flight with your
                        favourite seat
                                    </span>
                </div>
                <div class="seat-wrapper make_relative ">
                    <div class="plane-desc font16">
                        <p class="airline-name"></p>
                        <p>JetAircraft 737-800</p>
                    </div>
                    <div class="plane-wrapper make_relative clearfix">
                        <img src="//jsak.mmtcdn.com/flights/assets/media/cockpit.67e1d282.png" alt="cockpit"
                            class="cockpit-img" />
                        <div class="seatmap-wrapper ">
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="lavatory-wrap">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="galley-wrap">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="exit_icon left">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="exit_icon right">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">A</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">B</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">C</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">D</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">E</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">F</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">1</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat-not-available"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(238, 68, 35)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(238, 68, 35)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">1</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">2</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186,218,255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat-not-available"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(238, 68, 35)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(238, 68, 35)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(238, 68, 35)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">2</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">3</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat-not-available"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat-not-available"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">3</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">4</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat-not-available"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">4</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">5</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">5</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">6</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">6</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">7</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">7</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">8</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">8</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">9</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">9</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">10</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">10</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">11</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">11</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">12</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">12</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">14</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">14</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row wing-start ">
                                <span class="flight-wings left"
                                    style={{ width: "250px", left: "1160px" }}></span>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">15</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186 ,218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">15</div>
                                    </div>
                                </div><span class="flight-wings right"
                                    style={{ width: "250px", left: "1160px" }}></span>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">16</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">16</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">17</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(123, 184, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">17</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">18</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">18</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row wing-end">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">19</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">19</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">20</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">20</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">21</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">21</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">22</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">22</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">23</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">23</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">24</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">24</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">25</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">25</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">26</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">26</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">27</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">27</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">28</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">28</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">29</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">29</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">30</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">30</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">31</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">31</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">32</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">32</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">33</div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="make_relative">
                                        <div class="seat_block cursor_pointer "
                                            style={{ backgroundColor: "rgb(186, 218, 255)" }}></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_label">33</div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="galley-wrap">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="lavatory-wrap">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="lavatory-wrap">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="galley-wrap">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="seat_row ">
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="exit_icon left">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="empty_cell"></div>
                                </div>
                                <div class="seat_col">
                                    <div class="exit_icon right">
                                        <img src="data:image/png;base64,"
                                            alt="" />
                                    </div>
                                </div>
                                <div class="seat_col">
                                    <div>
                                        <div class="seat_empty_label"></div>
                                    </div>
                                </div>
                            </div>
                        </div><img src="//jsak.mmtcdn.com/flights/assets/media/tail.a786df74.png"
                            alt="flight tail" class="tail-img" />
                    </div>
                </div>
            </div>
        </div>
    );
}

export default FlightAncillarySeats;
